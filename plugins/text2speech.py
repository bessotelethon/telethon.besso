...
async def getVoice(message, matches):
    tmp = await message.reply("❏︙رجاء الانتظار...")
    export_file = (
        "tmp/"
...
async def run(msg, matches, chat_id, step, crons=None):
    ...
    if msg.is_reply and matches[0] == "ترجمه بصمه":
...
plugin = {
    "name": "",
    "desc": "",
    "usage": ["/ترجمه بصمه + الاسم كمثال ترجمه بصمه a بيسو"],
    "run": run,
    "sudo": True,
    "patterns": ["^[!/#]ترجمه بصمه (a|e) (.+)$", "^[!/#](ترجمه بصمه) (a|e)$"],
}
